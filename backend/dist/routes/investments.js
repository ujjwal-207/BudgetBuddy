...
router.get('/:id/transactions', async (req, res) => {
    try {
        const { id } = req.params;
        const userId = (0, auth_1.getUserId)(req);
        const result = await pool_1.default.query(`SELECT t.*
       FROM investment_transactions t
       JOIN investments i ON i.id = t.investment_id
       WHERE t.investment_id = $1
         AND i.user_id = $2
       ORDER BY transaction_date DESC`, [id, userId]);
        res.json(result.rows);
    }
    catch (error) {
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const userId = (0, auth_1.getUserId)(req);
        const { name, type, invested_amount, units, buy_price, current_price, current_value, notes, purchase_date } = req.body;
        const initialValue = current_value ?? invested_amount;
        const investmentResult = await client.query(`INSERT INTO investments
       ...
       RETURNING *`, [
        ...
        await client.query(`INSERT INTO investment_transactions
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const result = await pool_1.default.query(`UPDATE investments
       SET ${fields.join(', ')}
       WHERE id = $${paramIndex}
...
router.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const userId = (0, auth_1.getUserId)(req);
        await pool_1.default.query(`DELETE FROM investment_transactions
       WHERE investment_id = $1
         AND EXISTS (SELECT 1 FROM investments WHERE id = $1 AND user_id = $2)`, [id, userId]);
        await pool_1.default.query('DELETE FROM transfers WHERE investment_id = $1 AND user_id = $2', [id, userId]);
        await pool_1.default.query('DELETE FROM investments WHERE id = $1 AND user_id = $2', [id, userId]);
...
router.post('/:id/transaction', async (req, res) => {
    ...
    try {
        ...
        const investmentResult = await client.query(`SELECT id
       FROM investments
       WHERE id = $1
         AND user_id = $2`, [id, userId]);
        ...
        const txResult = await client.query(`INSERT INTO investment_transactions
       (investment_id, type, units, price_per_unit, total_amount, notes)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`, [id, type, units, price_per_unit, total_amount, notes || null]);
        if (type === 'buy') {
         ...
         WHERE id = $3
           AND user_id = $4`, [total_amount, units || 0, id, userId]);
        }
        await client.query('COMMIT');

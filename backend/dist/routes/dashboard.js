...
const getCashflowSummary = async (userId, month) => {
    const [incomeResult, transferResult] = await Promise.all([
        pool_1.default.query(`SELECT COALESCE(SUM(amount), 0) AS total_income
       FROM income
       WHERE user_id = $2
         AND DATE_TRUNC('month', allocation_month) = DATE_TRUNC('month', $1::date)`, [month, userId]),
       ...
       FROM transfers
       WHERE user_id = $2
         AND DATE_TRUNC('month', effective_month) = DATE_TRUNC('month', $1::date)`, [month, userId])
    ]);
...
router.get('/', async (req, res) => {
    try {
        ...
        const [categorySummary, dailySpending, totalResult, budgetResult, keywordsResult, cashflowSummary] = await Promise.all([
            pool_1.default.query(`SELECT c.name, c.icon, c.color,
           ...
           ORDER BY total_spent DESC`, [month, userId]),
            pool_1.default.query(`SELECT DATE(date) AS day, COALESCE(SUM(amount), 0) AS daily_total
           FROM expenses
           WHERE user_id = $1
             AND date >= CURRENT_DATE - INTERVAL '30 days'
           GROUP BY DATE(date)
           ORDER BY day`, [userId]),
            pool_1.default.query(`SELECT COALESCE(SUM(amount), 0) AS total
           FROM expenses
           WHERE user_id = $2
             AND DATE_TRUNC('month', date) = DATE_TRUNC('month', $1::date)`, [month, userId]),
            pool_1.default.query(`SELECT COALESCE(SUM(COALESCE(mcb.budget_amount, c.monthly_budget, 0)), 0) AS total_budget
           FROM categories c
           LEFT JOIN monthly_category_budgets mcb
             ON mcb.category_id = c.id
            AND DATE_TRUNC('month', mcb.month) = DATE_TRUNC('month', $1::date)
           WHERE c.user_id = $2`, [month, userId]),
            pool_1.default.query(`SELECT description, COUNT(*) AS frequency, AVG(amount) AS avg_amount
           FROM expenses
           WHERE user_id = $1
...
router.get('/health-score', async (req, res) => {
    try {
        ...
        const [expensesResult, budgetResult, cashflowSummary] = await Promise.all([
            pool_1.default.query(`SELECT e.amount, e.is_impulse, e.item_type, c.name AS category_name
         FROM expenses e
         JOIN categories c ON e.category_id = c.id
         WHERE e.user_id = $2
           AND DATE_TRUNC('month', e.date) = DATE_TRUNC('month', $1::date)`, [month, userId]),
            pool_1.default.query(`SELECT COALESCE(SUM(COALESCE(mcb.budget_amount, c.monthly_budget, 0)), 0) AS total_budget
         FROM categories c
         LEFT JOIN monthly_category_budgets mcb
           ON mcb.category_id = c.id
...
router.get('/streak', async (req, res) => {
    try {
        const userId = (0, auth_1.getUserId)(req);
        const currentStreakResult = await pool_1.default.query(`
    ...
    `, [userId]);
        const longestStreakResult = await pool_1.default.query(`
      WITH consecutive_days AS (
        SELECT
          DATE(date) AS expense_date,
          DATE(date) - ROW_NUMBER() OVER (ORDER BY DATE(date)) AS grp

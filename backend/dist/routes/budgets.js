...
router.post('/distribute', async (req, res) => {
    ...
    try {
        ...
        for (const [categoryName, percentage] of Object.entries(selectedDistribution)) {
            ...
            const updateResult = await client.query(`INSERT INTO monthly_category_budgets (month, category_id, budget_amount, updated_at)
         ...
         RETURNING *`, [selectedMonth, category.id, budgetAmount]);
            updates.push({
                ...updateResult.rows[0],
...
router.get('/suggest', async (req, res) => {
    try {
        (0, auth_1.getUserId)(req);
        const incomeAmount = parseFloat(req.query.income || '0');
        const investmentAmount = parseFloat(req.query.investment || '0');
        const availableBudget = Math.max(incomeAmount - investmentAmount, 0);
        const suggestions = Object.fromEntries(Object.entries(distributions).map(([type, categories]) => [
            type,
            {
                description: type === 'conservative'
                    ? 'Needs-focused'
                    : type === 'balanced'
                        ? 'Balanced approach'
                        : 'Lifestyle-focused',
                budgets: Object.fromEntries(Object.entries(categories).map(([category, percentage]) => [
                    category,
...
router.get('/plan', async (req, res) => {
    try {
        ...
        const [summary, categoriesResult] = await Promise.all([
            ...
            pool_1.default.query(`SELECT c.id, c.name, c.icon, c.color,

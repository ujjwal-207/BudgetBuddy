...
const inferInvestmentId = (value) => {
    if (!value || !value.startsWith('investment:'))
        return null;
...
router.get('/', async (req, res) => {
    try {
       ...
       LIMIT $2`, [selectedMonth, parseInt(limit, 10), userId]);
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const userId = (0, auth_1.getUserId)(req);
        const { from_account, to_account, amount, description, transfer_date, effective_month, transfer_type, investment_id, from_account_id, to_account_id } = req.body;
        const resolvedInvestmentId = investment_id || inferInvestmentId(to_account) || inferInvestmentId(from_account);
        ...
        const transferResult = await client.query(`INSERT INTO transfers
       ...
       RETURNING *, DATE_TRUNC('month', effective_month)::date AS effective_month`, [
        ...
        if (from_account_id) {
         ...
         WHERE id = $2`, [amount, from_account_id]);
        }
        if (to_account_id) {
            await client.query(`UPDATE accounts
         SET current_balance = current_balance + $1
         WHERE id = $2`, [amount, to_account_id]);
        }
        if (resolvedInvestmentId && resolvedType === 'investment_contribution') {
         ...
         WHERE id = $2`, [amount, resolvedInvestmentId]);
        }
        if (resolvedInvestmentId && resolvedType === 'investment_withdrawal') {
            const investmentResult = await client.query(`SELECT invested_amount, COALESCE(current_value, invested_amount) AS current_value
         FROM investments
         WHERE id = $1`, [resolvedInvestmentId]);
         ...
         WHERE id = $3`, [principalReduction, amount, resolvedInvestmentId]);
        }
...
router.get('/summary/monthly', async (req, res) => {
    try {
       ...
       ORDER BY total_transferred DESC`, [selectedMonth, userId]);
        res.json(result.rows);
    }
    catch (error) {

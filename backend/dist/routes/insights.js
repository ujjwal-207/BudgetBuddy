...
router.get('/weekly', async (req, res) => {
    try {
        ...
        const categorySpending = await pool_1.default.query(`
    ...
    `, [userId]);
        if (categorySpending.rows.length > 0) {
            ...
            const totalResult = await pool_1.default.query(`
      ...
      `, [userId]);
        ...
        const nightPurchases = await pool_1.default.query(`
    ...
    `, [userId]);
        ...
        const bestSpend = await pool_1.default.query(`
    ...
    `, [userId]);
        ...
        const savingOpp = await pool_1.default.query(`
    ...
    `, [userId]);
        ...
        const moodSpending = await pool_1.default.query(`
      SELECT 
        mood,
      ...
      GROUP BY mood
    `, [userId]);
        const stressedRow = moodSpending.rows.find((r) => r.mood?.toLowerCase() === 'stressed' || r.mood?.toLowerCase() === 'bored');
        if (stressedRow) {
            const stressedTotal = parseFloat(stressedRow.total);
            const normalResult = await pool_1.default.query(`
...
router.get('/trends', async (req, res) => {
    try {
        ...
        const trends = await pool_1.default.query(`
      SELECT 

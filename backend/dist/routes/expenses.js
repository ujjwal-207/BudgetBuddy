...
const impulseDetector_1 = require("../utils/impulseDetector");
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        let { amount, account_id, category_id, category_name, description, item_name, item_type, is_need, longevity, mood, payment_method, date, is_impulse, natural_input } = req.body;
        // Handle natural language input
        if (natural_input) {
            const parsed = (0, parser_1.parseNaturalLanguage)(natural_input);
        ...
        if (category_name && !category_id) {
            const catResult = await client.query('SELECT id FROM categories WHERE name = $1 AND user_id = $2', [category_name, userId]);
            if (catResult.rows.length > 0) {
                category_id = catResult.rows[0].id;
            }
            else {
                // Create new category if doesn't exist
                const newCatResult = await client.query(`INSERT INTO categories (user_id, name, icon, color, monthly_budget) 
           VALUES ($1, $2, '📁', '#999999', 0) 
           RETURNING id`, [userId, category_name]);
        ...
        if (is_impulse === undefined) {
            const impulseCheck = await (0, impulseDetector_1.checkImpulsePurchase)(userId, amount, category_id, mood, date ? new Date(date) : new Date());
            is_impulse = impulseCheck.isImpulse;
        }
        // Get category spending for quality score
        const categorySpentResult = await client.query(`SELECT COALESCE(SUM(amount), 0) as total
       FROM expenses
       WHERE category_id = $1
       AND user_id = $2
       AND DATE_TRUNC('month', date) = DATE_TRUNC('month', CURRENT_DATE)`, [category_id, userId]);
        const categorySpent = parseFloat(categorySpentResult.rows[0].total);
        // Get category budget
        const categoryBudgetResult = await client.query('SELECT monthly_budget FROM categories WHERE id = $1 AND user_id = $2', [category_id, userId]);
        const categoryBudget = categoryBudgetResult.rows[0].monthly_budget
            ? parseFloat(categoryBudgetResult.rows[0].monthly_budget)
            : undefined;
        // Calculate quality score
        const quality_score = (0, scorer_1.calculateQualityScore)({
            category_id,
...
router.patch('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const userId = (0, auth_1.getUserId)(req);
        const { amount, category_id, description, item_name, item_type, is_need, longevity, mood, is_impulse, date, payment_method, account_id } = req.body;
        const existingExpense = await pool_1.default.query('SELECT amount, account_id FROM expenses WHERE id = $1 AND user_id = $2', [id, userId]);
        ...
        try {
            ...
            const result = await client.query(`UPDATE expenses e
         SET ${fields.join(', ')}
         ...
         RETURNING e.*`, values);
            ...
            if (previous.account_id) {
                await client.query(`UPDATE accounts
           SET current_balance = current_balance + $1
           WHERE id = $2`, [previous.amount, previous.account_id]);
            }
            if (newAccountId) {
                await client.query(`UPDATE accounts
           SET current_balance = current_balance - $1
           WHERE id = $2`, [newAmount, newAccountId]);
            }
            await client.query('COMMIT');
            const hydratedResult = await client.query(`SELECT e.*, c.name as category_name, c.icon as category_icon, c.color as category_color,
                a.name as account_name, a.icon as account_icon
         FROM expenses e
         JOIN categories c ON e.category_id = c.id
         LEFT JOIN accounts a ON e.account_id = a.id
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        ...
        if (expenseResult.rows.length > 0 && expenseResult.rows[0].account_id) {
            await client.query(`UPDATE accounts
         SET current_balance = current_balance + $1
         WHERE id = $2`, [expenseResult.rows[0].amount, expenseResult.rows[0].account_id]);
        }
...
router.post('/bulk', async (req, res) => {
    ...
    try {
        ...
        for (const expense of expenses) {
            const { amount, category_id, category_name, description, item_name, item_type, is_need, longevity, mood, payment_method, date } = expense;
...
router.post('/:id/reflect', async (req, res) => {
    try {
        ...
        const existing = await pool_1.default.query(`SELECT r.id
       FROM reflections r
       ...
       WHERE r.expense_id = $1
        ...
        if (existing.rows.length > 0) {
            ...
            const result = await pool_1.default.query(`UPDATE reflections 
         SET verdict = $2, created_at = NOW()
         WHERE expense_id = $1
           AND EXISTS (SELECT 1 FROM expenses WHERE id = $1 AND user_id = $3)
         RETURNING *`, [id, verdict, userId]);
            res.json(result.rows[0]);
        }
        else {
            // Create new reflection
            const result = await pool_1.default.query(`INSERT INTO reflections (expense_id, verdict) 
         SELECT $1, $2

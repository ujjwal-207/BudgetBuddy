  ...
  try {
    ...
    const result = await pool.query(
      `SELECT *,
       ...
       ORDER BY date DESC
       LIMIT $2`,
      [monthOrCurrent(month as string | undefined), parseInt(limit as string, 10), userId]
    );
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const result = await client.query(
      `INSERT INTO income (user_id, amount, source, category, description, date, account_id, allocation_month, is_regular)
       VALUES ($1, $2, $3, $4, $5, $6, $7, DATE_TRUNC('month', $8::date)::date, $9)
       RETURNING *, DATE_TRUNC('month', allocation_month)::date AS allocation_month`,
      [userId, amount, source, category || null, description || null, incomeDate, account_id || null, allocationMonth, is_regular || false]
    );
    ...
    if (incomeResult.rows.length > 0 && incomeResult.rows[0].account_id) {
      await client.query(
        `UPDATE accounts
         SET current_balance = current_balance - $1
         WHERE id = $2`,
        [incomeResult.rows[0].amount, incomeResult.rows[0].account_id]
      );
    }

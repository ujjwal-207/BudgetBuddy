...
const normalizeStatus = (principalAmount: number, repaidAmount: number) => (
  repaidAmount >= principalAmount ? 'paid' : 'open'
);
  ...
  try {
    ...
    if (account_id) {
      ...
      await client.query(
        ...
        [accountDelta, account_id, userId]
      );
    }
...
router.post('/:id/repay', async (req, res) => {
  ...
  try {
    ...
    const existingLoan = await client.query(
      `SELECT id, principal_amount, repaid_amount, account_id, direction
       FROM loans
    ...
    const updatedLoan = await client.query(
      `UPDATE loans
       ...
       RETURNING *`,
      [nextRepaidAmount, nextStatus, destinationAccountId, repayment_date || new Date(), id, userId]
    );
    ...
    if (destinationAccountId) {
      ...
      await client.query(
        `UPDATE accounts
         SET current_balance = current_balance + $1
         WHERE id = $2 AND user_id = $3`,
        [accountDelta, destinationAccountId, userId]
      );
    }

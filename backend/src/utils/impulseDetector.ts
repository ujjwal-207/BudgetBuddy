...
export async function checkImpulsePurchase(
  userId: number,
  amount: number,
  categoryId: number,
  mood?: string | null,
  date: Date = new Date()
): Promise<ImpulseCheck> {
...
export async function getImpulseExpensesThisMonth(userId: number): Promise<any[]> {
...
export function formatImpulseWarning(reasons: string[]): string {
